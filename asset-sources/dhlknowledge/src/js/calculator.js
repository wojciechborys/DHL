(function ($, Config, Calc) {
  ...
  calcFields.filter('[data-calc-field="calculate"]').on('click', function (evt) {
    ...
    if ($(".calculator__result").length) {
      if (country == "") {
        $('.content-calc__calculator .content-calc__input--country').addClass('error');
      }
      else {
        $('.content-calc__calculator .content-calc__input--country').removeClass('error');
      }
    }
    ...
    resultsAppend = function () {
      ...
      if ($(".calculator__result").length) {
          zl = gr > 0 ? parseInt(cost['walk'] - gr) : cost['walk'];
          zl = zl / 100;
          if (!zl) {
            zl = gr = '--';
          }
